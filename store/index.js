...
export default {
    state: () => ({
      userPosition: ['manager', 'designer', 'project manager', 'developer'],
      users: [],
      simpleUser: {},
      usersInfo: []
    }),
        ...
        async getUsers ({ commit, state }) {
          ...
          if (response?.status === 200) {
            for (const user of response.data.data) {
              const element = { ...user, position: state.userPosition[Math.floor(Math.random() * state.userPosition.length)]}
              newResponse.push(element)
            }
          }
          commit('SET_DATA', { users: newResponse })
        },
        async getSimpleUser (context, id) {
            const response = await axios.get(`https://reqres.in/api/users/${ id }`)
            if (response?.status === 200)
              context.commit('SET_DATA', { simpleUser: response.data.data })
          },
        async getUsersInfo (context, id) {
            const response = await axios.get(`https://jsonplaceholder.typicode.com/users`)
            if (response?.status === 200)
              context.commit('SET_DATA', { usersInfo: response.data })
          },
    },
    getters: {
      userById: state => id => state.usersInfo.find(user => user.id === id)
    }
}
